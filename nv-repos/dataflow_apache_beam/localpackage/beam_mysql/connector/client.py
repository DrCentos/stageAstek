...
from localpackage.beam_mysql.connector.errors import MySQLClientError
...
class MySQLClient:
    ...
    def rough_counts_estimator(self, query: str) -> int:
        ...
        with _MySQLConnection(self._config) as conn:
            ...
            if total_number <= 0:
                raise localpackage.mysql.connector.errors.Error(f"Failed to estimate total number of records. Query: {count_query}")
            else:
                return total_number
...
class _MySQLConnection:
    ...
    def __enter__(self):
        try:
            self.conn = localpackage.mysql.connector.connect(**self._config)
            return self.conn
        except MySQLConnectorError as e:
