...
bucket = "nvabucket"#client.get_bucket(getenv('AIP_BUCKET'))
...
original_string = "gs://nvabucket/model"#getenv('AIP_STORAGE_URI') #"gs://nvabucket/model"
...
BUCKET_PATH = "gs://nvabucket/model"#getenv('AIP_MODEL_DIR')  # "gs://nvabucket/model/aiplatform-custom-training-2023-05-04-13:58:50.370/model/"
...
if len(blob_names) > 1:
    print("retrain")
    retrain = True
    blobs_benign = bucket.list_blobs(prefix='{}/{}/'.format("benign","retrain"), delimiter='/')
    blobs_malignant = bucket.list_blobs(prefix='{}/{}/'.format("malignant","retrain"), delimiter='/')
else :
    retrain = False
...
for blob in filesbenign:
    ...
    #TODO : supprimer l'image (blob) pour ne pas a la réutiliser lors d'un prochain entrainement
...
if retrain:
    ...
    print("old_model_name :", old_model_name)
...
datagen = ImageDataGenerator(
    rescale=1. / 255,
    # Add other data augmentation options as needed
)
